...
if __name__ == '__main__':
    rospy.init_node('distance_estimator_odom')

    # get the name of the follower vehicle
    follower = rospy.get_param("~follower","autovehicle")
    leader = rospy.get_param("~leader","car1")
    disttopicname = rospy.get_param("~dist_topic","/estimator/distance")
    angletopicname = rospy.get_param("~angle_topic","/estimator/angle")
...
#            rospy.loginfo("t_tmp={0}".format(t_tmp))
            ...
            (trans,rot) = listener.lookupTransform('{0}/base_footprint'.format(follower), '{0}/base_footprint'.format(leader), rospy.Time())
            dist=norm(trans[0:1])
